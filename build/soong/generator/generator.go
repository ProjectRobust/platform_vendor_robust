...
func init() {
	android.RegisterModuleType("robust_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = robustExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = robustExpandVariables(ctx, dep_file)
	...
	cmd := robustExpandVariables(ctx, String(g.properties.Cmd))
