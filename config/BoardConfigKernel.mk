...
KERNEL_MAKE_FLAGS += -j$(shell prebuilts/tools-robust/$(HOST_PREBUILT_TAG)/bin/nproc --all)
...
TOOLS_PATH_OVERRIDE := \
    LD_LIBRARY_PATH=$(BUILD_TOP)/prebuilts/tools-robust/$(HOST_PREBUILT_TAG)/lib:$$LD_LIBRARY_PATH \
    PERL5LIB=$(BUILD_TOP)/prebuilts/tools-robust/common/perl-base

ifeq ($(KERNEL_ARCH),arm64)
  # Add 32-bit GCC to PATH so that arm-linux-androidkernel-as is available for CONFIG_COMPAT_VDSO
  TOOLS_PATH_OVERRIDE += PATH=$(BUILD_TOP)/prebuilts/tools-robust/$(HOST_PREBUILT_TAG)/bin:$(KERNEL_TOOLCHAIN_arm):$$PATH
else
  TOOLS_PATH_OVERRIDE += PATH=$(BUILD_TOP)/prebuilts/tools-robust/$(HOST_PREBUILT_TAG)/bin:$$PATH
endif
